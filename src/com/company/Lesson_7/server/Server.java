package com.company.Lesson_7.server;
...
public class Server {
    ...
    public synchronized void privateMessage(String sender, String name, String message) {
        for (ClientHandler ch : clientHandlers) {
            if (ch.getRecord().getName().equalsIgnoreCase(name)) {
                String[] split = message.split("\\s");
                String privateMessage = message.substring(split[0].length() + split[1].length() + 1);
                ch.sendMessage(String.format("Private message from %s: %s", sender, privateMessage));
            }
        }
    }
}
